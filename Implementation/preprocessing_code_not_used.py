...
class preprocessing_code():
    ...
    def load_images_and_labels_custom(self):
        ...
        for path in os.listdir(self.mainapp_obj.dataset_path):
            img_path=os.path.join(self.mainapp_obj.dataset_path, path)
            ...
            lbl_path = os.path.join(self.mainapp_obj.training_dir, label_file)
            ...
            self.mainapp_obj.loaded_label.append(individual_label)
    ...
    def preprocess_and_save(self):
        ...
        """
        for image_filename in os.listdir(self.mainapp_obj.dataset_path):
            # Load image
            image_path = os.path.join(self.mainapp_obj.dataset_path, image_filename)
            ...
            if image is not None:
                ...
                processed_image_path = os.path.join(self.mainapp_obj.training_dir_images, image_filename)
        ...
        for label_filename in os.listdir(self.mainapp_obj.dataset_path):
            label_path = os.path.join(self.mainapp_obj.dataset_path, label_filename)
            # No processing needed for labels in this case, but can be added if required.
            shutil.copy(label_path, os.path.join(self.mainapp_obj.dataset_path, label_filename))

    def dataset_split(self):
        # Define the paths for images and labels directories
        self.mainapp_obj.dataset_path = os.path.abspath(self.mainapp_obj.dataset_path)  # Make sure it's an absolute path
        ...
        os.makedirs(self.mainapp_obj.training_dir_images, exist_ok=True)
        os.makedirs(self.mainapp_obj.validation_dir_images, exist_ok=True)
        os.makedirs(self.mainapp_obj.testing_dir_images, exist_ok=True)

        os.makedirs(self.mainapp_obj.training_dir_label, exist_ok=True)
        os.makedirs(self.mainapp_obj.validation_dir_label, exist_ok=True)
        os.makedirs(self.mainapp_obj.testing_dir_label, exist_ok=True)
        ...
        move_file(training_images, self.mainapp_obj.training_dir_images, self.mainapp_obj.training_dir_label)
        move_file(validation_images, self.mainapp_obj.validation_dir_images, self.mainapp_obj.validation_dir_label)
        move_file(testing_images, self.mainapp_obj.testing_dir_images, self.mainapp_obj.testing_dir_label)
        ...
        def create_yaml_config():
            ...
            train: {self.mainapp_obj.training_dir_images}
            val: {self.mainapp_obj.validation_dir_images}
            test: {self.mainapp_obj.testing_dir_images}

            nc: 3
            names: ["No Tumor", "Middle Tumor", "Severe Tumor"]
            """

            yaml_path = os.path.join(self.mainapp_obj.dataset_path, "tumor_detection.yaml")
        ...
        self.mainapp_obj.yaml_path = create_yaml_config()