...
root = tk.Tk()
root.title("Brain Tumor Detection Interface")
root.geometry("1200x800")

# Global variable to store dataset path and processing status
dataset_path = ""
loaded_images = []

# Initialize the preprocessing steps status
...
# Create a label to display dataset information
dataset_info_label = tk.Label(root, text="No dataset loaded")
...
def upload_dataset():
    ...
    if dataset_path:
        ...
        loaded_images = backend.load_images_from_folder(dataset_path)
        
        # Get the number of images
        num_images = len(loaded_images)
        
        # Update the display with dataset information
        dataset_info_label.config(text=f"Dataset loaded from: {dataset_path}\nNumber of images: {num_images}")
        
        # Show a message box with the details
        messagebox.showinfo("Dataset Uploaded", f"Dataset uploaded from: {dataset_path}\nNumber of images: {num_images}")
    else:
...
def preprocess_data():
    ...
    progress_bars = {}
    for step in preprocessing_status:
    ...
    def run_preprocessing():
        ...
        for step in preprocessing_status:
            ...
            preprocessing_status[step].set(False)
...
def augment_data():
    ...
    augment_window = tk.Toplevel(root)
    augment_window.title("Data Augmentation")
    augment_window.geometry("1200x800")

    build_augmentation_interface(augment_window)

def build_augmentation_interface(augment_window):
    # Image preview frame
    ...
    def apply_rotation():
        ...
        for var, idx in check_vars:
            if var.get():
                ...
                loaded_images[idx] = (name, rotated_img)
...
augment_button = tk.Button(root, text="Augment Data", command=augment_data)
augment_button.pack(pady=10)
